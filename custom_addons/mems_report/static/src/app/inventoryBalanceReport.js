...
function runReport() {
    ...
    $.ajax({
        url: '/api/inventory/balance',
        type: 'get',
        async: false,
        success: function (res) {
            $.each(res.rows, function (index, data) {
                sum_stock_qty += data.stock_qty;
                sum_cost_price += data.cost_price;
                ...
                tr += '<tr>' +
                    ...
                    '<td class="text-right">' + numberWithCommas(data.cost_price, 2) + '</td>' +
                    '<td class="text-right">' + numberWithCommas(data.total_cost, 2) + '</td>' +
                    '</tr>';
            });
            // make tbody data
            ...
            tr = '<tr>' +
                ...
                '<th class="text-right">' + numberWithCommas(sum_cost_price, 2) + '</th>' +
                '<th class="text-right">' + numberWithCommas(sum_total_cost, 2) + '</th>' +
                '</tr>';
